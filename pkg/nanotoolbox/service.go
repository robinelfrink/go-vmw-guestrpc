...
func (s *Service) Request(request []byte) ([]byte, error) {
	...
	result, _, err := s.rpci.Request(request)

	return result, err
}

// GetString wraps nanotoolbox' rpci.GetString for demarcation/protection.
func (s *Service) InfoGet(key, defaultValue string) (string, error) {
	return s.rpci.InfoGet(key, defaultValue)
}

// SetString wraps nanotoolbox' rpci.SetString for demarcation/protection.
func (s *Service) InfoSet(key, value string) (bool, error) {
	return s.rpci.InfoSet(key, value)
}

// SetGuestInfo wraps nanotoolbox' rpci.SetGuestInfo for demarcation/protection.
func (s *Service) SetGuestInfo(kind GuestInfoID, data []byte) {
	s.rpci.SetGuestInfo(kind, data)
}
...
func (s *Service) startChannels() error {
	err := s.tclo.Start()
	if err != nil {
		return err
	}
